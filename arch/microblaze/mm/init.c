...
#else
	m_start = PAGE_ALIGN((unsigned long)_end);
#endif
	m_end   = DDR_SDRAM_HIGHADDR;

	min_low_pfn = PFN_UP(DDR_SDRAM_BASEADDR);
	max_mapnr = PFN_DOWN(DDR_SDRAM_HIGHADDR);
...
{
	high_memory = (void *)DDR_SDRAM_HIGHADDR;
