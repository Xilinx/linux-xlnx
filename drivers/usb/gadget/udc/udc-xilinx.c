		       ...
		       u8 *bufferptr, u32 bufferlen)
{
	...
	if (!ep->curbufnum && !ep->buffer0ready) {
		...
		eprambase = (u32 __force *)(udc->addr + ep->rambase);
		if (ep->is_in) {
			__memcpy_toio(eprambase, bufferptr, bytestosend);
			udc->write_fn(udc->addr, ep->offset +
				      XUSB_EP_BUF0COUNT_OFFSET, bufferlen);
		} else {
			__memcpy_toio(bufferptr, eprambase, bytestosend);
		}
		/*
...
static int __xudc_ep0_queue(struct xusb_ep *ep0, struct xusb_req *req)
{
	...
	if (udc->setup.bRequestType & USB_DIR_IN) {
		...
		length = req->usb_req.actual = min_t(u32, length,
						     EP0_MAX_PACKET);
		__memcpy_toio(corebuf, req->usb_req.buf, length);
	...
	__must_hold(&udc->lock)
{
	...
	__memcpy_toio(&setup, ep0rambase, 8);
...
static void xudc_ep0_out(struct xusb_udc *udc)
{
	...
	case DATA_PHASE:
		...
		__memcpy_toio(buffer, ep0rambase, bytes_to_rx);
...
static void xudc_ep0_in(struct xusb_udc *udc)
{
	...
	case DATA_PHASE:
		if (!bytes_to_tx) {
			...
			__memcpy_toio(ep0rambase, buffer, length);
		}
