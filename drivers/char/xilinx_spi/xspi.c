/* $Id: xspi.c,v 1.4 2007/04/13 00:55:47 wre Exp $ */
/******************************************************************************
*
...
int XSpi_Start(XSpi * InstancePtr)
{
	 ...
	 */
	XSPI_WRITE_IIER(InstancePtr->BaseAddr, XSP_INTR_DFT_MASK);
	 ...
	 */
	XSPI_GINTR_ENABLE(InstancePtr->BaseAddr);
...
int XSpi_Stop(XSpi * InstancePtr)
{
	 ...
	 */
	XSPI_GINTR_DISABLE(InstancePtr->BaseAddr);
...
void XSpi_Reset(XSpi * InstancePtr)
{
	 ...
	 */
	XSPI_RESET(InstancePtr->BaseAddr);
}
		  ...
		  u8 *RecvBufPtr, unsigned int ByteCount)
{
	 ...
	 */
	IntrEnable = XSPI_READ_IIER(InstancePtr->BaseAddr);
	XSPI_WRITE_IIER(InstancePtr->BaseAddr,
...
void XSpi_InterruptHandler(void *InstancePtr)
{
	 ...
	 * TX_EMPTY interrupt.
	 */
	IntrStatus = XSPI_READ_IISR(SpiPtr->BaseAddr);
	XSPI_WRITE_IISR(SpiPtr->BaseAddr, IntrStatus);
	...
	if (IntrStatus & XSP_INTR_TX_EMPTY_MASK) {
		...
		if (SpiPtr->RemainingBytes > 0) {
			 ...
			 */
			IntrEnable = XSPI_READ_IIER(SpiPtr->BaseAddr);
			XSPI_WRITE_IIER(SpiPtr->BaseAddr,
					      IntrEnable &
