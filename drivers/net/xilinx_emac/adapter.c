 ...
 */
static irqreturn_t
xenet_interrupt(int irq, void *dev_id)
{
...
descriptor_init(struct net_device *dev)
{
    ...
    lp->desc_space = dma_alloc_coherent(NULL, dftsize, &lp->desc_space_handle, GFP_ATOMIC);
    if (lp->desc_space == 0) {
        return -1;
    }
...
remove_head_dev(void)
{
	...
	dma_free_coherent(NULL, lp->desc_space_size, lp->desc_space, lp->desc_space_handle);
    }
...
probe(int index)
{
    static const unsigned long remap_size
        = CONFIG_XILINX_ETHERNET_0_HIGHADDR - CONFIG_XILINX_ETHERNET_0_BASEADDR + 1;
    struct net_device *dev;
    struct net_local *lp;
    XEmac_Config *cfg;
    unsigned int irq;
...
#error Edit this file to add more devices.
	...
	XStatus Result;
	u16 reg;
	Result = XEmac_PhyRead(&lp->Emac, 31, MII_BMCR, &reg);
    ...
    printk(KERN_INFO
           ...
           dev->name, index, (unsigned int)cfg->PhysAddress, (unsigned int)cfg->BaseAddress, dev->irq);
