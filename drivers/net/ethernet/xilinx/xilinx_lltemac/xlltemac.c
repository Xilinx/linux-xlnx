...
int XLlTemac_SetMacAddress(XLlTemac *InstancePtr, void *AddressPtr)
{
  ...
  xdbg_printf(XDBG_DEBUG_GENERAL,
	      "XLlTemac_SetMacAddress: setting mac address to "	\
  ...
  /* Normal mode */
  XLlTemac_WriteIndirectReg(InstancePtr->Config.BaseAddress,
			    XTE_UAW0_OFFSET, MacAddr0);
  XLlTemac_WriteIndirectReg(InstancePtr->Config.BaseAddress,
			    XTE_UAW1_OFFSET, MacAddr1);
  
#ifdef CONFIG_XILINX_LL_TEMAC_EXT
  /* Ext mode */
  XLlTemac_WriteReg(InstancePtr->Config.BaseAddress,
		    XTE_UAWL0_OFFSET, MacAddr0);
  XLlTemac_WriteReg(InstancePtr->Config.BaseAddress,
		    XTE_UAWU0_OFFSET, MacAddr1);
#endif
...
void XLlTemac_GetMacAddress(XLlTemac *InstancePtr, void *AddressPtr)
{
  ...
  MacAddr0 = XLlTemac_ReadIndirectReg(InstancePtr->Config.BaseAddress,
				      XTE_UAW0_OFFSET);
  MacAddr1 = XLlTemac_ReadIndirectReg(InstancePtr->Config.BaseAddress,
				      XTE_UAW1_OFFSET);
#else
  MacAddr0 = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
			      XTE_UAWL0_OFFSET);
  MacAddr1 = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
			      XTE_UAWU0_OFFSET);
#endif
  
  Aptr[0] = (u8) MacAddr0;
  Aptr[1] = (u8) (MacAddr0 >> 8);
  Aptr[2] = (u8) (MacAddr0 >> 16);
  Aptr[3] = (u8) (MacAddr0 >> 24);
  Aptr[4] = (u8) MacAddr1;
  Aptr[5] = (u8) (MacAddr1 >> 8);
}
...
int XLlTemac_SetOptions(XLlTemac *InstancePtr, u32 Options)
{
	...
	Reg = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
			       XTE_RAF_OFFSET);
	XLlTemac_WriteReg(InstancePtr->Config.BaseAddress,
			  XTE_RAF_OFFSET, Reg | XTE_RAF_NEWFNCENBL_MASK);
#endif
	...
	if (Options & XTE_PROMISC_OPTION) {
		...
		XLlTemac_WriteIndirectReg(InstancePtr->Config.BaseAddress,
					  XTE_AFM_OFFSET, Reg);
#ifdef CONFIG_XILINX_LL_TEMAC_EXT
		/* Full promiscuous = no mcast filtering */
		Reg = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
				       XTE_RAF_OFFSET);
		/* Set regs */
		Reg &= ~XTE_RAF_EMULTIFLTRENBL_MASK;
		XLlTemac_WriteReg(InstancePtr->Config.BaseAddress,
				  XTE_RAF_OFFSET, Reg);
#endif
	}
	...
	xdbg_printf(XDBG_DEBUG_GENERAL,
		    "setOptions: %08x / %08x RAF: %08x AFM: %08x\n",
		    InstancePtr->Options, Options,
