/*
 * Xilinx DRM KMS Mixer Framebuffer helper
 *
 *  Copyright (C) 2017 Xilinx, Inc.
 *
 *  Author: Jeffrey Mouroux <jmouroux@xilinx.com>
 *
 *  Based on Xilinx DRM KMS Framebuffer driver by Hyun Kwon <hyunk@xilinx.com>
 *  Copyright (C) 2013
 *
...
static int xvmixer_drm_fb_create_handle(struct drm_framebuffer *base_fb,
...
static struct drm_framebuffer_funcs xilinx_drm_fb_funcs = {
	...
	.create_handle	= xvmixer_drm_fb_create_handle,
};
...
/**
 * xvmixer_drm_fb_get_gem_obj - Get CMA GEM object for framebuffer
...
/**
 * xvmixer_drm_fb_set_config - synchronize resolution changes with fbdev
				   ...
				   struct drm_fb_helper_surface_size *sizes)
{
	...
	mode_cmd.pitches[0] = ALIGN(sizes->surface_width * bytes_per_pixel,
				    fbdev->align);
	mode_cmd.pixel_format = xvmixer_drm_get_format(drm);
	...
	for (i = 0; i < num_planes; i++) {
		...
		bpp = drm_format_plane_cpp(mode_cmd.pixel_format, i);
		...
		width /= (i ? hsub : 1);
		height /= (i ? vsub : 1);
		height *= fbdev->vres_mult;
		mode_cmd.pitches[i] = ALIGN(width * bpp, fbdev->align);
		size[i] = mode_cmd.pitches[i] * height;
		screen_data += size[i];
		obj[i] = drm_gem_cma_create(drm, size[i]);
		if (IS_ERR(obj[i]))
			return PTR_ERR(obj[i]);
	}
	...
	fbdev->fb = xilinx_drm_fb_alloc(drm, &mode_cmd, obj, num_planes);
	...
	drm->mode_config.fb_base = (resource_size_t)obj[0]->paddr;
	fbi->screen_base = (char __iomem *)(obj[0]->vaddr + offset);
	fbi->fix.smem_start = (unsigned long)(obj[0]->paddr + offset);
	fbi->screen_size = screen_data;
	fbi->fix.smem_len = screen_data;
...
err_drm_gem_cma_free_object:
	for (i = 0; i < num_planes; i++)
		drm_gem_cma_free_object(&(obj[i]->base));
	return ret;
}
...
/**
 * xvmixer_drm_fb_init - Allocate and initializes the Xilinx framebuffer
 * @drm: DRM device
...
/**
 * xvmixer_drm_fb_restore_mode - Restores initial framebuffer mode
...
/**
 * xvmixer_drm_fb_create - (struct drm_mode_config_funcs *)->fb_create callback
 * @drm: DRM device
		     ...
		     const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	if (!fb->base.bits_per_pixel)
		fb->base.bits_per_pixel =
			xvmixer_drm_format_bpp(mode_cmd->pixel_format);
...
/**
 * xvmixer_drm_fb_hotplug_event - Poll for hotpulug events
 * @fb_helper: drm_fb_helper struct, may be NULL
 *
