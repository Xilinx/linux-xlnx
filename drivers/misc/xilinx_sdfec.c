...
static int
xsdfec_clk_init(struct platform_device *pdev, struct xsdfec_clks *clks)
{
	int err;
	...
	clks->dout_words_clk = devm_clk_get(&pdev->dev,
	...
	if (err) {
		dev_err(&pdev->dev,
			"failed to enable dout_words_clk (%d)", err);
...
static void
xsdfec_disable_all_clks(struct xsdfec_clks *clks)
{
